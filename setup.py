#!/usr/bin/python3
# -*- coding: utf-8 -*-
# Created by: python.exe -m py2exe TOPAS_report_parser.py -O -c -b 0 -W setup.py
...
class Target(object):
    ...
    def __init__(self, **kw):
        ...
        self.company_name = "China Resource Cement, R&D Center"
        self.copyright = "Copyright China Resource Cement, R&D Center © 2016"
        self.legal_copyright = self.copyright
        # self.legal_trademark = ""
        self.product_version = "1.0.0.0"
        self.product_name = "TOPAS report parser"
...
TOPAS_report_parser = Target(
    ...
    script="TOPAS_report_parser.py", # path of the main script

    # Allows to specify the basename of the executable, if different from 'TOPAS_report_parser'
    # dest_base = "TOPAS_report_parser",

    # Icon resources:[(resource_id, path to .ico file), ...]
    icon_resources=[(1, r"TOPAS_report_parser.ico")],

    other_resources = [(RT_MANIFEST, 1, (manifest_template % dict(prog="TOPAS_report_parser", level="asInvoker")).encode("utf-8")),
...
'''
ntpath numbers opcode operator
            optparse os posixpath py_compile quopri random re reprlib select selectors shutil sre_compile sre_constants
            sre_parse stat string stringprep struct subprocess tarfile tempfile textwrap threading token tokenize traceback
            types warnings weakref zipextimporter zipfile _bootlocale _bz2 _collections_abc _dummy_thread _osx_support
            _strptime _threading_local _tkinter _weakrefset abc argparse ast base64 bz2 calendar codecs contextlib
 copy copyreg datetime
'''
py2exe_options = dict(
    packages = [],
    excludes = "_ssl Image PIL _abcoll lxml openpyxl.tests \
            win32evtlog win32evtlogutil \
            pydoc_data _hashlib pyreadline unicodedata _socket \
            _decimal _lzma _elementtree \
            pyexpat _ctypes pdb \
            difflib dis doctest dummy_threading getopt gettext gzip \
            lzma \
            email html http unittest".split(),
##    ignores = "dotblas gnosis.xml.pickle.parsers._cexpat mx.DateTime".split(),
##    dll_excludes = "tcl86t.dll tk86t.dll python34.dll".split(),
    optimize=1,
